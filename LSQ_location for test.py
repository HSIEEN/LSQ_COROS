...
def least_squares(sat_pos, pseudoranges, weights=1, x_hat=np.array([0, 0, 0, 0])):
    ...
    while norm2_dx > 1e-2 and iterations < 30:
        ...
        mean_dp = np.linalg.norm(dp)/len(dx)
        ratio = max_dp/mean_dp
        G[:, 0:3] = -(sat_pos - x_hat[:3]) / norms[:, None]
        if (mean_dx < 1e2) and (len(sat_pos) > 7) and (max_dp > 25):
            # if max_dp > 1e2:
            # del_flag = True
    ...
    # if (res > 2) and len(res_sat_pos) > 7 and iterations > 8:
    #     # recursive call?
...
if __name__ == '__main__':
    ...
    root = tk.Tk()
    root.withdraw()
    print('---------COROS LSQ position solving v1.3.0-----------')
    ...
    with open(file[:-4] + '_PR_observables.txt', 'r') as fPR:
        for pline in fPR:
            ...
            # if TOW < 527300.0 or TOW > 527350.0:
            #     continue
            for key in list(line_dict.keys()):
                ...
                if len(line_dict[key]) != 6 or line_dict[key][1] == 0 or ((key[0] % 2 != band) and band != 2):
                    del line_dict[key]
    ...
    if band == 0:
        ...
        create_kml(file[:-4] + '_COROS_weighted_LSQ_l1_l5', coord)
