...
def least_squares(sat_pos, pseudoranges, weights=1, x_hat=np.array([0, 0, 0, 0])):
    ...
    while increment_x > 1e-3:
        ...
        if (increment_x < 1e3) and (len(sat_pos) > 10) and (max_dp > 30):
...
if __name__ == '__main__':
    ...
    with open(file[:-4] + '_PR_observables.txt', 'r') as fPR:
        for pline in fPR:
            ...
            for key in list(line_dict.keys()):
                # if dictionary data are incomplete or pseudorange is 0
                if len(line_dict[key]) != 6 or line_dict[key][1] == 0:
                    del line_dict[key]
    ...
    create_kml(file[:-4] + '_COROS_weighted_LSQ_l1_and_l5', coord)
